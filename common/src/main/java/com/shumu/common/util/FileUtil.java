...
import com.shumu.common.constant.NumberConstant;
...
public class FileUtil {
     ...
     * @return
     */
    public static String getImgFileExtendName(byte[] photoByte) {
        String strFileExtendName = TextConstant.JPG;
        String b01 = photoByte[0] + "" + photoByte[1];
        String b13 = photoByte[1] + "" + photoByte[NumberConstant.TWO] + "" + photoByte[NumberConstant.THREE];
        String b05 = photoByte[0] + "" + photoByte[1] + "" + photoByte[NumberConstant.TWO] + ""
                + photoByte[NumberConstant.THREE] + "" + photoByte[NumberConstant.FOUR] + ""
                + photoByte[NumberConstant.FIVE];
        String b69 = photoByte[NumberConstant.SIX] + "" + photoByte[NumberConstant.SEVEN] + ""
                + photoByte[NumberConstant.EIGHT] + "" + photoByte[NumberConstant.NINE];
        if (TextConstant.GIF_87A_ASCII.equals(b05) || TextConstant.GIF_98A_ASCII.equals(b05)) {
            strFileExtendName = TextConstant.GIF;
        } else if (TextConstant.JPG_ASCII.equals(b69)) {
            strFileExtendName = TextConstant.JPG;
        } else if (TextConstant.BMP_ASCII.equals(b01)) {
            strFileExtendName = TextConstant.BMP;
        } else if (TextConstant.PNG_ASCII.equals(b13)) {
            strFileExtendName = TextConstant.PNG;
        }
    ...
    private static String getFileType(MultipartFile file) throws Exception {
        Map<String, String> fileTypeMap = getFileTypeMap();
    ...
    private static Map<String, String> getFileTypeMap() {
        Map<String, String> fileTypeMap = new HashMap<>(32);
    ...
    public static String getFileName(String fileName) {
        ...
        // 替换上传文件名字的特殊字符
        fileName = StringUtil.specialCharFilter(fileName);
        return fileName;
    }
}
